...
class Solution2 {
public:
    int minsum(vector<vector<int>>& tri, vector<vector<int>>& t, int n, int i, int j, int sum) {
        if(i==n) return sum;
        else {
            sum+=tri[i][j];
            int lchild=minsum(tri, t, n, i+1, j, sum);  //pathsum of left child
            int rchild=minsum(tri, t, n, i+1, j+1, sum);    //pathsum of right child
            t[i][j]=min(lchild, rchild);
            return t[i][j];
        }
    }
    int minimumTotal(vector<vector<int>>& triangle) {
        ...
        return minsum(triangle, t, r, 0, 0, 0);
    }
};
//top down tabulation->
class Solution3 {
